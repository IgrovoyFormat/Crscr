...
async def forward_message(event):
   ...
   # --- 4. ОБРАБОТКА И ОТПРАВКА ---
    ...
    try:
        ...
        if text:
            # 1. Удаляем мусорные слова и эмодзи (от них остаются пустые скобки)
            text = re.sub(r'(?i)(Scanner:|Trader:|Dolbaeb Trade|😎|Link)', '', text)
            
            # 2. Удаляем любые ссылки на Telegram
            text = re.sub(r'(https?://)?(www\.)?t\.me/[^\s]+', '', text)
            
            # 3. Строгая очистка (если включена)
            ...
            # --- 4.1 НОВЫЙ БЛОК: Бронебойная очистка остатков скобок ---
            # Если остался полезный текст без ссылки, например [Текст]() -> превращаем в Текст
            text = re.sub(r'\[(.*?)\]\(\s*\)', r'\1', text)
            
            # Уничтожаем пустые комбинации: [](), []( , [ ] (, [] и т.д.
            text = re.sub(r'\[\s*\]\s*[()]*', '', text)
            
            # Удаляем целые строки, в которых не осталось НИЧЕГО, кроме скобок и пробелов
            # Это 100% добьет ту висячую строку с "[](" в самом низу сообщения
            text = re.sub(r'^[\[\]()\s]+$', '', text, flags=re.MULTILINE)
            # ------------------------------------------------------------
            
            # 4.2 Удаляем двойные звездочки
            text = text.replace('**', '')
